...
export default function Navbar() {
  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 py-3 transition-all  border-b border-white/10 duration-300 `}
    >
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navLinks.map((link, index) => (
              ...
              >
                <Link
                  href={link.href}
                  className="text-white/70 tracking-widest text-md font-medium  hover:text-white transition-all duration-300 relative group"
                >
                  {link.name}
